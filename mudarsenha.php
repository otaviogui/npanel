...
$mensagem1 = "Error ao alterar a senha";

	include("global/conexao.php");	include("global/conexao.php");
if(isset($_POST['login']) && isset($_POST['senha']) && isset($_POST['nsenha']) && isset($_POST['type']) && $_POST['type'] == "1") {
	...
	$query = mysql_query("SELECT * FROM usuarios WHERE email='$login' AND senha='$senha'") or die(mysql_error());
	$n_rows = mysql_num_rows($query);
	$row = mysql_fetch_assoc($query);
	if($n_rows > "0"){
		$update = mysql_query("UPDATE usuarios SET senha='$nsenha' WHERE email='$login'") or die(mysql_error());
